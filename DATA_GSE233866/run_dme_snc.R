...
# DME requires ONE shared network/module definition across both groups being
# compared -- unlike ModulePreservation (which explicitly builds a reference
# network then tests it against a different query dataset), FindDMEs compares
# hMEs computed from a single network across two cell subsets of the SAME
# object. So this pools lesioned + intact SNc cells into one network, rather
# than reusing the two separately-built (differently-labeled) networks from
# build_snc_lesioned_network.R / build_snc_intact_network.R.
p <- list(
  h5seurat_path = "/shared/DATA_GSE233866/seurat_lesion_intact_SNc_VTA.rds",
  out_dir       = "/shared/DATA_GSE233866/results/snc_dme",
  cell_type_col = "cell_type",
  group_by      = c("cell_type", "sample_id"),
  group_name    = "Dopaminergic Neurons",
  wgcna_name    = "cacna1d_mouse_snc_combined"
)
p <- with_defaults(p)
dir.create(p$out_dir, recursive = TRUE, showWarnings = FALSE)

obj <- subset_condition(load_seurat(p$h5seurat_path), "region", "SNc")
log_mem("after load+subset(SNc, both conditions)")
...
PlotDendrogram(obj, main = "Mouse SNc (lesioned + intact combined) — gene dendrogram", wgcna_name = p$wgcna_name)
dev.off()
...
# -----------------------------------------------------------------
# DME: lesioned vs intact, within this one combined SNc network
# -----------------------------------------------------------------
