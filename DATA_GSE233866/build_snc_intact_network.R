...
p <- list(
  ...
  out_dir       = "/shared/DATA_GSE233866/results/snc_intact_network",
  cell_type_col = "cell_type",
  group_by      = c("cell_type", "sample_id"),
  group_name    = "Dopaminergic Neurons",
  wgcna_name    = "cacna1d_mouse_snc_intact",
...
rm(obj_snc); gc()
log_mem("after load+subset(SNc, intact)")
...
PlotDendrogram(obj, main = "Mouse SNc (intact) — Dopaminergic Neuron gene dendrogram", wgcna_name = p$wgcna_name)
dev.off()
...
p_heatmap <- ggplot(df_long, aes(x = module, y = sample_id, fill = z_hME)) +
  ...
  labs(x = "Module", y = "Animal", title = "Mouse SNc (intact) — mean harmonized module eigengene per animal")
