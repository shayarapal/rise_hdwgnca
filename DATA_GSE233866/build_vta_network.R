...
p <- list(
  ...
  out_dir       = "/shared/DATA_GSE233866/results/vta_network",
  cell_type_col = "cell_type",
  group_by      = c("cell_type", "sample_id"),
  group_name    = "Dopaminergic Neurons",
  wgcna_name    = "cacna1d_mouse_vta",
...
log_mem("after load+subset(VTA)")
cat("n_cells:", ncol(obj), "\n")

obj <- setup_and_metacells(obj, p)
log_mem("after setup_and_metacells(VTA)")

obj <- test_soft_powers(obj, p)
log_mem("after test_soft_powers(VTA)")
...
soft_power <- if (!is.null(rec$recommended_power) && !is.na(rec$recommended_power)) {
  ...
  stop("No soft power reached the scale-free threshold for VTA -- inspect soft_power_table.csv")
}
...
log_mem("after ConstructNetwork(VTA)")

png(file.path(p$out_dir, "dendrogram.png"), width = 2400, height = 1400, res = 200)
PlotDendrogram(obj, main = "Mouse VTA — Dopaminergic Neuron gene dendrogram", wgcna_name = p$wgcna_name)
dev.off()

obj <- ScaleData(obj, features = GetWGCNAGenes(obj, p$wgcna_name), verbose = FALSE)
log_mem("after ScaleData(VTA)")
...
log_mem("after ModuleEigengenes+Connectivity(VTA)")

write_module_outputs(obj, p)
log_mem("after write_module_outputs(VTA)")
...
pdf(file.path(p$out_dir, "module_correlogram.pdf"), width = 14, height = 13)
ModuleCorrelogram(obj, wgcna_name = p$wgcna_name)
dev.off()
png(file.path(p$out_dir, "module_correlogram.png"), width = 2800, height = 2600, res = 200)
ModuleCorrelogram(obj, wgcna_name = p$wgcna_name)
dev.off()
log_mem("after ModuleCorrelogram(VTA)")
...
log_mem("after ModuleExprScore+FeaturePlot(VTA)")
...
p_heatmap <- ggplot(df_long, aes(x = module, y = sample_id, fill = z_hME)) +
  ...
  labs(x = "Module", y = "Animal", title = "Mouse VTA — mean harmonized module eigengene per animal")
