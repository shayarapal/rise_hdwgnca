...
p <- list(
  ...
  out_dir       = "/shared/DATA_GSE233866/results/vta_intact_network",
  cell_type_col = "cell_type",
  group_by      = c("cell_type", "sample_id"),
  group_name    = "Dopaminergic Neurons",
  wgcna_name    = "cacna1d_mouse_vta_intact",
...
# Mirrors build_snc_intact_network.R exactly, region="VTA" instead of "SNc".
# Restrict to VTA first, THEN to the intact condition within VTA.
obj_full <- load_seurat(p$h5seurat_path)
obj_vta  <- subset_condition(obj_full, "region", "VTA")
rm(obj_full); gc()
obj <- subset_condition(obj_vta, p$condition_col, p$ref_group)
rm(obj_vta); gc()
log_mem("after load+subset(VTA, intact)")
...
PlotDendrogram(obj, main = "Mouse VTA (intact) — Dopaminergic Neuron gene dendrogram", wgcna_name = p$wgcna_name)
dev.off()
...
p_heatmap <- ggplot(df_long, aes(x = module, y = sample_id, fill = z_hME)) +
  ...
  labs(x = "Module", y = "Animal", title = "Mouse VTA (intact) — mean harmonized module eigengene per animal")
