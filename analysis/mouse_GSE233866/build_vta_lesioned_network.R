...
p <- list(
  ...
  out_dir       = "/shared/DATA_GSE233866/results/vta_lesioned_network",
  cell_type_col = "cell_type",
  group_by      = c("cell_type", "sample_id"),
  group_name    = "Dopaminergic Neurons",
  wgcna_name    = "cacna1d_mouse_vta_lesioned",
...
obj_snc  <- subset_condition(obj_full, "region", "VTA")
rm(obj_full); gc()
obj <- subset_condition(obj_snc, p$condition_col, p$ref_group)
rm(obj_snc); gc()
log_mem("after load+subset(VTA, lesioned)")
...
PlotDendrogram(obj, main = "Mouse VTA (lesioned) — Dopaminergic Neuron gene dendrogram", wgcna_name = p$wgcna_name)
dev.off()
...
p_heatmap <- ggplot(df_long, aes(x = module, y = sample_id, fill = z_hME)) +
  ...
  labs(x = "Module", y = "Animal", title = "Mouse VTA (lesioned) — mean harmonized module eigengene per animal")
