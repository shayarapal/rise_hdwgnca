...
p <- list(
  ...
  out_dir       = "/shared/results/control_network_final",
  cell_type_col = "cell_type",
  group_by      = c("cell_type", "sample_id"),
  group_name    = "Dopaminergic Neurons",
  wgcna_name    = "cacna1d_control",
  soft_power    = 5,
...
log_mem("after load+subset(control)")

obj <- setup_and_metacells(obj, p)
log_mem("after setup_and_metacells(control)")

obj <- test_soft_powers(obj, p)
log_mem("after test_soft_powers(control)")
...
log_mem("after ModuleExprScore(control, UCell)")
...
log_mem("after ModuleFeaturePlot(control, scores)")
...
log_mem("after RunEnrichr(control)")
...
log_mem("after EnrichrBarPlot(control)")
