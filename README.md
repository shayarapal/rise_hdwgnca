...
The **scientific results** were not generated by AI. Every number in `analysis_results/`
...
Everything except the analysis outputs lives under `src/`. `analysis_results/` is kept at
the repository root so that results are separable from the code that produced them.
...
analysis_results/     # pipeline outputs — kept at root, separate from the code
DATA_GSE233866/       # raw mouse data
DATA_UNZIPPED/        # unpacked inputs and standalone R build scripts
local_data/           # gitignored; bind-mounted into both containers at /shared
